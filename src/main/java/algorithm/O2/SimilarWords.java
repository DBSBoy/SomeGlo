 ...
 * 相似单词
 *
 * 给定一个可存储若干单词的字典，找出指定单词的所有相似单词，并且按照单词名称从小到大排序输出。单词仅包括字母，但可能大小写并存 (大写不一定只出现在首字母)。相似单词说明: 给定一个单词X，如果通过任意交换单词中字母的位置得到不同的单词Y，那么定义Y是X的相似单词，如abc、bca即为相似单词 (大小写是不同的字母，如a和A算两个不同字母)。字典序只排序: 大写字母<小写字母。同样大小写的字母，遵循26字母顺序大小关系。即A<B<C<..<X<Y<Z<a<b<c<...<x<y<z.如Bac<aBc<acB<cBa.
 * 输入描述
 * 第一行为给定的单词个数N (N为非负整数)
